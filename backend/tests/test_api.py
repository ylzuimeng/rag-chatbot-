...
class TestQueryEndpoint:
    ...
    def test_query_with_new_session(
        self, test_app, mock_rag_system, successful_query_response, mock_search_response
    ):
        """Test query endpoint creates new session when none provided."""
        # Setup mocks
        mock_rag_system.query.return_value = (successful_query_response, mock_search_response)
    ...
    def test_query_with_existing_session(
        ...
        response = client.post(
            "/api/query", json={"query": "Tell me more about MCP", "session_id": sample_session_id}
        )
    ...
    def test_query_with_empty_sources(
        self, test_app, mock_rag_system, successful_query_response, sample_session_id
    ):
    ...
    def test_query_with_error_response(
        self, test_app, mock_rag_system, error_query_response, sample_session_id
    ):
        ...
        # Execute
        client = TestClient(test_app)
        response = client.post("/api/query", json={"query": "Invalid query"})
    ...
    def test_query_with_exception(self, test_app, mock_rag_system):
    ...
    def test_query_with_missing_query_field(self, test_app):
        ...
        client = TestClient(test_app)
        response = client.post(
            "/api/query", json={"session_id": "test-session"}  # Missing 'query' field
        )
...
class TestCoursesEndpoint:
    ...
    def test_get_course_stats_success(self, test_app, mock_rag_system):
    ...
    def test_get_course_stats_empty_database(self, test_app, mock_rag_system):
    ...
    def test_get_course_stats_single_course(self, test_app, mock_rag_system):
    ...
    def test_get_course_stats_with_exception(self, test_app, mock_rag_system):
    ...
    def test_get_course_stats_response_structure(self, test_app, mock_rag_system):
...
class TestAPIIntegrationScenarios:
    ...
    def test_consecutive_queries_same_session(
        ...
        response2 = client.post(
            "/api/query", json={"query": "Tell me more", "session_id": first_session_id}
        )
    ...
    def test_query_and_get_courses_workflow(
        ...
        query_response = client.post(
            "/api/query", json={"query": f"Tell me about {courses_data['course_titles'][0]}"}
        )
    ...
    def test_api_error_recovery(self, test_app, mock_rag_system, sample_session_id):
        ...
        mock_rag_system.session_manager.create_session.return_value = sample_session_id
        response2 = client.post("/api/query", json={"query": "Test query"})
        assert response2.status_code == 200
...
class TestAPIEdgeCases:
    ...
    def test_concurrent_requests(
        ...
        def make_request(query_id):
            try:
                response = client.post("/api/query", json={"query": f"Concurrent query {query_id}"})
